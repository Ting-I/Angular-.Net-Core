...
| `/crud` skill asks for Moq, `RowAuditWriter`, sticky `p-toolbar` | Moq no; `RowAuditWriter` yes, retrofitted later; the toolbar arrived in another form | Moq is not used in this repo. `RowAuditWriter` did not exist when this page was built and does now — `FeaturedPromoItemRepository` writes an audit row on every insert, update and delete, plus one per row a slot swap moved (the trip through slot 0 is scaffolding and is not audited on its own); see the 異動紀錄 section of `spec/conventions/backend.md`. A pinned action bar arrived later, as `.sticky-toolbar` around a `.page-header` rather than a `p-toolbar` (`spec/conventions/frontend.md`); this page predates it |
